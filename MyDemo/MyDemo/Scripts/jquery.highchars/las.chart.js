/**
* @图表控件(chart)
* @fileoverview 依赖于HighCharts
* @author linchang(林昶)
* @version v1.0
* @date 2014-03-10
*/

delete Array.prototype.contain;
...
; (function ($) {
    ...
    $.fn.createChart = function (options) {
        ...
        var defaults = {
            ...
            labelFormat: 0,
            //指定标签显示的格式，0: 不显示，1: 显示Y值， 2：自动计算百分比并显示
            categories: [],
            //X轴数据
            series: [],
            //Y轴数列
            ...
            cssNoData: 'nodata',
            //无数据时样式名
            isCompareSeries: false,
            //判断两个数列是否为对比数列
            autoxAxisDataType: false,
            //是否自动识别X轴数据类型，比如日期类型，以别进行相关个性化设置
        ...
        cOptions.chart.type = cOptions.chart.type || defaultChartType; (typeof options.title == 'object') ? cOptions.title = options.title : cOptions.title.text = options.title;
        ...
        $(this).removeClass(options.cssNoData); (!options.categories) && (cOptions.chart.type = 'pie'); //如options.categories为空，默认为饼图
        //设置X轴数据，
        if (options.categories && cOptions.chart.type != 'pie') {
            ...
            if (cOptions.xAxis.type !== 'datetime') {
                ...
                cOptions.xAxis.labels.formatter = cOptions.xAxis.labels.formatter ||
                function () {
                    var d = new Date(this.value);
                    var result = isNaN(d) ? this.value :
                    /*d.getFullYear() + '-' +*/
        ...
        if (options.labelFormat == 0) {
            ...
            cOptions.plotOptions.series.dataLabels.formatter = cOptions.plotOptions.series.dataLabels.formatter ||
            function () {
        ...
        if (cOptions.chart.type != 'pie') {
            cOptions.tooltip.formatter = function () {
                ...
                function (i, point) {
                    ...
                    s += '<tr><td style="padding: 2px 5px" >' + title + ' </td>' + '<td style="text-align: right;padding-left:15px">' + value + suffix + ' </td></tr>';
                });
                s += '</table>';
                return s;
            };
            ...
            cOptions.tooltip.formatter = cOptions.tooltip.formatter ||
            function () {
		...
		*/
        ...
        function toSeriesItem(ser) {
            ...
            for (var j in serData) {
				...
				*/
                isNotPieChart() && (hcPoint.marker = hcPoint.marker || {},
                typeof (hcPoint.marker.enabled) == 'undefined' && (hcPoint.marker.enabled = false));
